...
def get_user(member,other_member,is_creator):
    ...
    if is_creator == 1:
        ...
        text1 = user.first_name + ' ' + user.second_name
    bot.send_message(int(member), text=text1, reply_markup=keyboard)
...
while True:
    ...
    try:
        ...
        pass
    try:
        event = Events.select().where((Events.time == datetime.time(time1.hour, time1.minute)) & (Events.date == datetime.date(date.year, date.month, date.day)))
        for i in event:     # i - выбранное мероприятие
            for member in list(i.members.split()):
                bot.send_message(int(member),text='Ваше мероприятие "' + i.text + '" началось! Не забудьте поставить оценку приглашённым для рейтинга')
